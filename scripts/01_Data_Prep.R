...
library(tidygeocoder) # geocodifica
...
# ---------------TRASFORMAZIONE SPAZIALE E JITTERING---------------------------

# 1. Creazione oggetto spaziale con le coordinate originali (WGS84)
dati_sf <- st_as_sf(df_clean_coords, coords = c("long", "lat"), crs = 4326)

# 2. Conversione in UTM 32N (Metri) per le analisi GWR/MGWR
dati_sf_metri <- st_transform(dati_sf, crs = 32632)

# 3. JITTERING IN METRI
# Spostamento casuale di massimo 5 metri per evitare sovrapposizioni esatte
set.seed(999) # Seme casuale per riproducibilità
dati_sf_metri <- st_jitter(dati_sf_metri, amount = 5)

# 4. CONTROLLO SU MAPPA (Leaflet)
# Leaflet richiede coordinate in gradi (WGS84), quindi ritrasformiamo temporaneamente
# solo per la visualizzazione sulla mappa
dati_sf_mappa <- st_transform(dati_sf_metri, crs = 4326)
coords_mappa <- st_coordinates(dati_sf_mappa)

leaflet(dati_sf_mappa) %>%
  addTiles() %>%
  addCircleMarkers(
    lng = coords_mappa[,1], 
    lat = coords_mappa[,2], 
