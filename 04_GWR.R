...
dati_sf_metri <- st_transform(dati_lisa, crs = 32632)
...
# Creazione del dataset base per la GWR (usando SOLO il select sicuro)
dati_sf_std <- dati_sf_metri %>% 
  dplyr::select(ID, comune)

# Convertiamo in Spatial (formato richiesto da GWmodel)
dati_sp_std <- as(dati_sf_std, "Spatial")

cat("Dataset preparato e convertito in Spatial con successo!\n")
